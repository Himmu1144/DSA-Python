...
exp2 = '7-)((((8(3*9)+11+12))-8)))'
exp2 = list(exp2)

# creating stack instance
stack = Stack(len(exp1))   # size should be equal to the length of the expression
...
def expStatus(exp1):
    ...
    for i in exp1:
        ...
        if i == ')':
            count += 1
            if stack.isEmpty():
                remaining_elements = stack.top + 1
                return f"The Expression is Unbalanced by {remaining_elements}, Steps Taken - {count}"
            stack.pop()
...
status = expStatus(exp1)
print(status)

status = expStatus(exp2)
print(status)
